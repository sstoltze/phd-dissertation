...
\begin{definition}
  Given an invertible matrix $V = [v_1,\dots,v_m]$, the \textit{left
    flag} of $V$ is
  \begin{alignat*}{5}
    \mathrm{Fl_L}(V)\, &&= \Big(&\spa(v_1) &&\subset \spa(v_1,v_2)
    &&\subset \dots &&\subset \spa(v_1,\dots,v_m) = V\Big).
  \end{alignat*}
  The \textit{right flag} of $V$ is defined similarly as
  \begin{alignat*}{5}
    \mathrm{Fl_R}(V)\, &&= \Big(&\spa(v_m) &&\subset
    \spa(x_{v-1},x_v) &&\subset \dots &&\subset
    \spa(v_1,\dots,v_m) = V\Big). 
  \end{alignat*}
\end{definition}
Note that we could multiply any column of $V$ by a non-zero scalar and
...
change the left flag of $V$. And the right flag is unchanged by doing
